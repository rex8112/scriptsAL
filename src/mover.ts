...
import { IPosition, MapKey, MonsterKey } from "typed-adventureland";
...
class Mover
{
     ...
     * @returns {Promise<void>}
     */
    static async move_by_path(destination, nodeReached: Function | null = null)
    {
     ...
     */
    static async get_path(start: IPosition, destination: SmartMoveToDestination | {to: string})
    {
        ...
        else if("to" in destination || "map" in destination)
        {
            if(destination.to == "town")
                destination.to = "main";
            if(G.monsters[<MonsterKey>destination.to])
            {
                ...
                for(let name in G.maps)
                {
                    ...
                    (G.maps[<MapKey>name].monsters || []).forEach(function(pack){
                        if(pack.type != destination.to || G.maps[<MapKey>name].ignore ||  G.maps[<MapKey>name].instance)
                            return;
                        if(pack.boundaries)
                        {
